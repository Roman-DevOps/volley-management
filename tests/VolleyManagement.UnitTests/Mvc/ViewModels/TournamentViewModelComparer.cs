...
namespace VolleyManagement.UnitTests.Mvc.ViewModels
{
    ...
    internal static class TournamentViewModelComparer
    {
        public static void AssertAreEqual(TournamentViewModel expected, TournamentViewModel actual)
        {
            ...
            actual.TransferEnd?.Date.Should().Be(expected.TransferEnd?.Date, $"[Id:{expected.Id}]TransferEnd should be equal.");
            actual.TransferStart?.Date.Should().Be(expected.TransferStart?.Date, $"[Id:{expected.Id}]TransferStart should be equal.");
