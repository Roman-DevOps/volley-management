    ...
    public class GameServiceTests
    {
        ...
        public GameServiceTests()
        {
            ...
            _gamesByTournamentIdRoundsNumberQueryMock =
                new Mock<IQuery<ICollection<Game>, TournamentRoundsGameResultsCriteria>>();
            ...
            _gamesByTournamentIdInRoundsByNumbersQueryMock =
                new Mock<IQuery<ICollection<Game>, GamesByRoundCriteria>>();
        ...
        private readonly Mock<IQuery<ICollection<GameResultDto>, TournamentGameResultsCriteria>>
            _tournamentGameResultsQueryMock;

        private readonly Mock<IQuery<ICollection<Game>, TournamentRoundsGameResultsCriteria>>
            _gamesByTournamentIdRoundsNumberQueryMock;

        private readonly Mock<IQuery<TournamentScheduleDto, TournamentScheduleInfoCriteria>>
            _tournamentScheduleDtoByIdQueryMock;

        private readonly Mock<IQuery<Tournament, FindByIdCriteria>> _tournamentByIdQueryMock;

        private readonly Mock<IQuery<ICollection<Game>, GamesByRoundCriteria>>
            _gamesByTournamentIdInRoundsByNumbersQueryMock;

        private readonly Mock<IQuery<Game, GameByNumberCriteria>> _gameNumberByTournamentIdQueryMock;

        private readonly Mock<IQuery<ICollection<TeamTournamentDto>, FindByTournamentIdCriteria>>
            _tournamentTeamsQueryMock;

        private readonly Mock<IUnitOfWork> _unitOfWorkMock;

        private VolleyManagement.Services.GameService BuildSUT()
        {
            return new VolleyManagement.Services.GameService(
        ...
        private void MockAllTournamentQueries(TournamentScheduleDto testData)
        {
            ...
            MockGetTeamsInTournament(testData.Id,
        ...
        private static void AssertPlaceholdersAreUsed(ICollection<GameResultDto> actual, int numberOfGamesInFirstRound)
        {
            ...
            firstRoundGames.Count.Should().Be(numberOfGamesInFirstRound,
            ...
            string messagePrefix)
        {
            ...
            game.HomeTeamName.Should().Be($"{prefix}{upstreamHomeTeamGameNumber}",
                $"[GameId:{game.Id}] {messagePrefix} of upstream game should be set as Home team name");

            game.AwayTeamName.Should().Be($"{prefix}{upstreamAwayTeamGameNumber}",
        ...
        {
            Assert.NotNull(game.HomeTeamId);
            Assert.Null(game.AwayTeamId);
        }
        ...
        public void Create_CreateNotPermitted_ExceptionThrown()
        {
            ...
            VerifyCreateGame(testData, Times.Never(), Times.Never());
        ...
        public void Create_SameGameInRound_ExceptionThrown()
        {
            ...
            MockGetTournamentById(TOURNAMENT_ID,
                new TournamentScheduleDtoBuilder().WithScheme(TournamentSchemeEnum.One).Build());
        ...
        [Fact(Skip = "Bug in test need to be fixed")]
        ...
        public void Create_SecondFreeDayInDifferentRoundInDifferentDivision_GameCreated()
        {
            ...
            MockGetTeamsInTournament(TOURNAMENT_ID,
        ...
        /// <summary>
        ///     Test for EditGameResult() method with no permission for such action. The method has to throw
        ...
        public void GetPlayoffTournamentGames_FirstRoundGameHasNoTeams_PlaceholdersAreUsed()
        {
            ...
            MockGetTournamentById(TOURNAMENT_ID,
                new TournamentScheduleDtoBuilder().WithScheme(TournamentSchemeEnum.PlayOff).Build());
            MockGetTournamentResults(TOURNAMENT_ID, existingGames);
        ...
        public void GetPlayoffTournamentGames_NonFirstRoundGameHasOnlyOneTeams_GameDependenciesAreSetForMissingTeam()
        {
            ...
            game.HomeTeamName.Should().Be(TEAM_NAME,
                $"[GameId:{game.Id}] Team name should be set");
            game.AwayTeamName.Should().Be(
                $"Winner2",
        ...
        /// <summary>
        ///     Test for GetTournamentResults method. Game results of specified tournament are requested. Game results are
        ///     returned.
        /// </summary>
        [Fact]
        ...
        public void SwapRounds_AllGamesExist_GamesSwapped()
        {
            ...
            // Assert
            Assert.True(games.SequenceEqual(expectedGames,
