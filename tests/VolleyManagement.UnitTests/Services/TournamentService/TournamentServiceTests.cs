...
namespace VolleyManagement.UnitTests.Services.TournamentService
{
    ...
    public class TournamentServiceTests : IDisposable
    {
        ...
        private readonly Mock<IQuery<ICollection<Tournament>, GetAllCriteria>> _getAllQueryMock;
        private readonly Mock<IQuery<Tournament, FindByIdCriteria>> _getByIdQueryMock;
        private readonly Mock<IQuery<List<TeamTournamentDto>, FindByTournamentIdCriteria>> _getAllTournamentTeamsQuery;

        private readonly Mock<IQuery<ICollection<Division>, TournamentDivisionsCriteria>>
            _getAllTournamentDivisionsQuery;

        private readonly Mock<IQuery<ICollection<Group>, DivisionGroupsCriteria>> _getAllTournamentGroupsQuery;
        private readonly Mock<IQuery<List<TeamTournamentAssignmentDto>, GetAllCriteria>> _getAllGroupsTeamsQuery;
        private readonly Mock<IQuery<ICollection<Team>, GetAllCriteria>> _getAllTeamsQuery;
        private readonly Mock<IQuery<TournamentScheduleDto, TournamentScheduleInfoCriteria>> _getTorunamentDto;
        private readonly Mock<IQuery<Tournament, TournamentByGroupCriteria>> _getTournamentId;
        private readonly Mock<IQuery<ICollection<Tournament>, OldTournamentsCriteria>> _getOldTournamentsQuery;
        private readonly Mock<IUnitOfWork> _unitOfWorkMock = new Mock<IUnitOfWork>();

        private readonly Mock<TimeProvider> _timeMock = new Mock<TimeProvider>();

        private VolleyManagement.Services.TournamentService BuildSUT()
        {
            return new VolleyManagement.Services.TournamentService(
        ...
        {
            _getOldTournamentsQuery.Setup(tr => tr.Execute(It.IsAny<OldTournamentsCriteria>()))
        ...
        {
            _getAllTournamentTeamsQuery.Setup(tr => tr.Execute(It.IsAny<FindByTournamentIdCriteria>()))
                .Returns(testData);
        }

        private void MockGetAllTournamentDivisionsQuery(ICollection<Division> testData)
        {
            _getAllTournamentDivisionsQuery.Setup(tr => tr.Execute(It.IsAny<TournamentDivisionsCriteria>()))
                .Returns(testData);
        }
        ...
        private void MockGetAllTournamentTeamsQueryTwoCalls(List<TeamTournamentDto> firstCallTestData,
            List<TeamTournamentDto> secondCallTestData)
        {
        ...
        {
            _tournamentRepositoryMock.Verify(tr => tr.Add(It.Is<Tournament>(t => TournamentsAreEqual(t, tournament))),
                times);
        ...
        {
            _tournamentRepositoryMock.Verify(tr => tr.AddTeamToTournament(It.IsAny<int>(), It.IsAny<int>()),
                repositoryTimes);
        ...
        public void AddTeamsToTournament_AddTeamInSecondGroupOfSecondDivision_TeamIsAdded()
        {
            ...
            var teamsToAddInSecondDivision = new List<TeamTournamentDto>
                {new TeamTournamentDto {TeamId = SPECIFIC_TEAM_ID}};
        ...
        /// <summary>
        ///     Test for AddTeamsToTournament() method with no rights for such action. The method should throw
        ...
        /// <summary>
        ///     Test for Create() method. Tournament's division count is out of range. Exception is thrown during tournament
        ///     creation.
        /// </summary>
        [Fact]
        ...
        /// <summary>
        ///     Test for Create() method. Tournament's divisions do not have unique names. Exception is thrown during tournament
        ///     creation.
        /// </summary>
        [Fact]
        ...
        /// <summary>
        ///     Test for DeleteTeamFromTournament() method with no rights for such action. The method should throw
        ///     AuthorizationException
        ///     and shouldn't invoke Commit() method of IUnitOfWork.
        /// </summary>
        [Fact]
        ...
        public void FindById_Existing_TournamentFound()
        {
            ...
            // Assert
            TestHelper.AreEqual(tournament, actualResult, new TournamentComparer());
        }
