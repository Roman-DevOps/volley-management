...
namespace VolleyManagement.UnitTests.Services.TeamService
{
    ...
    public class TeamServiceTests
    {
        ...
        private readonly Mock<IQuery<Player, FindByFullNameCriteria>> _getPlayerByFullNameQueryMock;
        private readonly Mock<IQuery<Team, FindByCaptainIdCriteria>> _getTeamByCaptainQueryMock;
        private readonly Mock<IQuery<ICollection<Team>, GetAllCriteria>> _getAllTeamsQueryMock;
        private readonly Mock<IQuery<ICollection<Player>, TeamPlayersCriteria>> _getTeamRosterQueryMock;
        private readonly Mock<IUnitOfWork> _unitOfWorkMock;

        private VolleyManagement.Services.TeamService BuildSUT()
        {
            return new VolleyManagement.Services.TeamService(
        ...
        {
            _playerRepositoryMock.Verify(pr => pr.Update(It.Is<Player>(p => p.Id == playerId && p.TeamId == teamId)),
                times);
            _unitOfWorkMock.Verify(uow => uow.Commit(), times);
        }
        ...
        private string CreateInvalidTeamAchievements()
        {
            ...
            for (var i = 0; i < Constants.Team.MAX_ACHIEVEMENTS_LENGTH + 1; i++)
            {
        ...
        private string CreateInvalidTeamName()
        {
            ...
            for (var i = 0; i < Constants.Team.MAX_NAME_LENGTH + 1; i++)
            {
        ...
        private string CreateInvalidTeamCoachName()
        {
            ...
            for (var i = 0; i < Constants.Team.MAX_COACH_NAME_LENGTH + 1; i++)
            {
        ...
        public void Create_EmptyTeamName_ArgumentExceptionThrown()
        {
            ...
            var argExMessage = string.Format(
                TournamentResources.ValidationTeamName,
                Constants.Team.MAX_NAME_LENGTH);
            var testTeam = new TeamBuilder()
                .WithName(invalidName)
                .Build();
            var testPlayer = new PlayerBuilder().WithId(PLAYER_ID).Build();
            _getPlayerByIdQueryMock.Setup(pr => pr.Execute(It.Is<FindByIdCriteria>(cr => cr.Id == testPlayer.Id)))
                .Returns(testPlayer);
            MockGetAllTeamsQuery(CreateSeveralTeams());
        ...
        public void Create_InvalidTeamCoachNameNotAllowedLength_ArgumentExceptionThrown()
        {
            ...
            var argExMessage = string.Format(
                TournamentResources.ValidationCoachName,
        ...
        public void Create_PlayerIsNotCaptainOfExistingTeam_TeamCreated()
        {
            ...
            _getTeamByCaptainQueryMock.Setup(tq => tq.Execute(It.IsAny<FindByCaptainIdCriteria>()))
            ...
            // Assert
            Assert.Equal(newTeam.Id, captain.TeamId);
            VerifyCreateTeam(newTeam, Times.Once());
        }
        ...
        public void Create_TeamPassed_CaptainUpdated()
        {
            ...
            // Assert
            Assert.Equal(captain.TeamId, SPECIFIC_TEAM_ID);
            VerifyCreateTeam(newTeam, Times.Once());
        }
        ...
        public void Create_TeamPassed_TeamCreated()
        {
            ...
            // Assert
            Assert.NotEqual(newTeam.Id, UNASSIGNED_ID);
            VerifyCreateTeam(newTeam, Times.Once());
        }
        ...
        {
            // Arrange
            var expectedRoster = new PlayerServiceTestFixture().TestPlayers().Build().AsQueryable();
        ...
        public void Edit_CatchDalConcurrencyException_ThrowMissingEntityException()
        {
            ...
            // Act
            var sut = BuildSUT();
            Action act = () => sut.Edit(teamWithWrongId);

            //Assert
            act.Should().Throw<MissingEntityException>();
        }
        ...
        public void Edit_InvalidCaptainId_ThrowMissingEntityException()
        {
            ...
            // Act
            var sut = BuildSUT();
            Action act = () => sut.Edit(teamWithWrongCaptainId);
        ...
        public void UpdateRosterTeamId_PlayerAndTeamPassed_PlayerUpdated()
        {
            ...
            var secondPlayer = new PlayerBuilder().WithId(PLAYER_ID + 1).WithFirstName("Second")
                .WithTeamId(SPECIFIC_TEAM_ID).Build();
            var testRoster = new List<Player> {
                firstPlayer,
                secondPlayer
            };
        ...
        public void UpdateRosterTeamId_PlayerIsCaptainOfExistingTeam_ValidationExceptionThrown()
        {
            ...
            _getTeamByCaptainQueryMock.Setup(tr => tr.Execute(It.IsAny<FindByCaptainIdCriteria>()))
