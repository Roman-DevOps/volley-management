...
namespace VolleyManagement.UnitTests.Services.RequestsService
{
    ...
    public class RequestServiceTests : BaseTest
    {
        ...
        private readonly Mock<IQuery<ICollection<Request>, GetAllCriteria>> _getAllRequestsQueryMock;
        ...
        {
            _requestRepositoryMock.Verify(pr => pr.Add(It.Is<Request>(p => RequestAreEqual(p, request))), times,
                message);
            _unitOfWorkMock.Verify(uow => uow.Commit(), times);
        }
        ...
        public void Create_InvalidPlayerId_ExceptionThrown()
        {
            ...
            // Act => Assert
            Assert.Throws<ArgumentException>(() => sut.Create(EXISTING_USER_ID, INVALID_PLAYER_ID),
                "Player's id is wrong");
        }
